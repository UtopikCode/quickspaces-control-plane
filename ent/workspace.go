...
type Workspace struct {
	...
	// ActualState holds the value of the "actual_state" field.
	ActualState string `json:"actual_state,omitempty"`
	// ExecutionProfile holds the value of the "execution_profile" field.
...
func (*Workspace) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case workspace.FieldExecutionProfile:
			values[i] = new([]byte)
		case workspace.FieldID, workspace.FieldRepo, workspace.FieldOwner, workspace.FieldRef, workspace.FieldDesiredState, workspace.FieldActualState:
...
func (_m *Workspace) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case workspace.FieldActualState:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field actual_state", values[i])
			} else if value.Valid {
				_m.ActualState = value.String
			}
		case workspace.FieldExecutionProfile:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field execution_profile", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.ExecutionProfile); err != nil {
					return fmt.Errorf("unmarshal field execution_profile: %w", err)
				}
			}
...
func (_m *Workspace) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("actual_state=")
	builder.WriteString(_m.ActualState)
	builder.WriteString(", ")
	builder.WriteString("execution_profile=")
	builder.WriteString(fmt.Sprintf("%v", _m.ExecutionProfile))
	builder.WriteString(", ")
